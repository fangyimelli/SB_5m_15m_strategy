...
- 2026-03-03 Focus snapshot timeframe fix (focus_mode HUD all `—` regression):
  ...
  - In focus HUD snapshot mode, `stSnap/scoreASnap/scoreAPlusSnap/fvgMitSnap/bosDirSnap/inNySnap` now all use `ta.valuewhen(condFocusNY5, ..., 0)`.
  - `tradeDaySnap` now uses whole-day condition `ta.valuewhen(condFocusDay5, array.get(arrIsTradeDay,0), 0)` and no longer requires NY-session bars.
  - `foundSnap = not na(stSnap)` is now the only source for focus `—` fallback; not masked by current chart day context.
  - HUD debug line now prints `focusDayKey`, `foundSnap=Y/N`, and optional `lastNYBarTimeUsed` timestamp.
